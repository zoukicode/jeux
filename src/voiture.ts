const voiture = document.querySelectorAll('.voiture') as NodeListOf<SVGSVGElement>;
const boxs = document.querySelector('.box') as HTMLDivElement;
const p = document.querySelector('p') as HTMLParagraphElement;
const selecte = document.querySelector('select') as HTMLSelectElement;

let courserobot: number;
const normale: number = 900;
const moyen: number = 600;
const rapide: number = 300;
let time: number = 0;
...
function Game() {
  ...
  function ecoute(e: KeyboardEvent) {
    if (!playing) return;
